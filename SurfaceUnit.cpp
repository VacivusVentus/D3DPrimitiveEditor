...
void TSurfaceRender::ReInitRenderer()
{
	...
	ReleaseGUI(); // Останавливаем графический конвеер
	...
	scdesc.SampleDesc.Count = 4; // включам MSAA сглаживание
	...
	// ------------------------------------------------------
    //Так как отрисовка по двухмерным координатам, то буфер глубины не потребуется
	// ------------------------------------------------------

	auto func = [this](ID3D11DeviceContext * m_context, bool *working) // лямбда функция графичекого конвеера
	{
		float color[] =
		{1.0, 1.0, 1.0, 1.0};
