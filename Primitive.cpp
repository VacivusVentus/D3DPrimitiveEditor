...
{
	buffervs.params[1] = widthline; // толщина линии дл€ рассчета координат
	psBuffer.params[0] = 1.0f;
	for (int i = 2; i < widthline; i++)
	{
		psBuffer.params[0] *= 1.0f + 0.1f; // перасчет выдел€мых пикселей
	}
	psBuffer.params[0] = widthline;
	vars.getDeviceContext()->UpdateSubresource(cBufferPS, 0, 0, &psBuffer, 0, 0);
	if (fromang) // ќбновление значени€ угла из градусов в радианы или обратно
	{ // дл€ отображени€, редактировани€ и использование в шейдерах
