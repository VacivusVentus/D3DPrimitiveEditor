...
{
	psBuffer.params[0] = 1.0; // для отработки толщины линие дожна увеличиваться и длина стиля линии(точки)
	for (int i = 2; i < widthline; i++)
	{
		psBuffer.params[0] *= 1.0f / 30.0f + 1.0;
	}
	__updateResource(fromang);
	vars.getDeviceContext()->UpdateSubresource(cBufferPS, 0, 0, &psBuffer, 0, 0);
}

void Primitive::__updateResource(bool fromang)
{
	vars.getDeviceContext()->UpdateSubresource(cBufferPS, 0, 0, &psBuffer, 0, 0);
	if (fromang)
